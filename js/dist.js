...
function reviewsio_getAjax(userVaribles, carouselType, styles){
  ...
  var apiVersion =  apiBranch+apiTags;//+'&v='+apiVersionDate.getFullYear().toString()+ (apiVersionDate.getMonth() + 1).toString()+ apiVersionDate.getDate().toString()+ apiVersionDate.getHours().toString();
  //var apiVersion = '&v='+ Date.now();
  ...
  if(carouselType[0] == 'third_party'){
    ...
    if(carouselType.length == 1){
      apiUrl= userVaribles.api+'/timeline/data?type=store_third_party_review&store='+userVaribles.store+'&sort=date_desc&page=1&per_page='+extraNum+'&sku=&enable_avatars='+userVaribles.options.reviews.enable_avatars+'&include_subrating_breakdown=1'+apiVersion;
    }else{
      ...
      if(storeApi == 1){
        apiUrl= userVaribles.api+'/timeline/data?type=store_review&store='+userVaribles.store+'&sort=date_desc&page=1&per_page='+extraNum+'&sku=&enable_avatars='+userVaribles.options.reviews.enable_avatars+'&include_subrating_breakdown=1'+apiVersion+finalString;
      }else{
        apiUrl= userVaribles.api+'/timeline/data?type=store_third_party_review&store='+userVaribles.store+'&sort=date_desc&page=1&per_page='+extraNum+'&sku=&enable_avatars='+userVaribles.options.reviews.enable_avatars+'&include_subrating_breakdown=1'+apiVersion+finalString;
      }
    }
  }
      ...
      if(carouselType.length == 1){
        var storeApi = 0;
        apiUrl= userVaribles.api+'/timeline/data?type=product_review&store='+userVaribles.store+'&sort=date_desc&page=1&per_page='+extraNum+'&sku='+userVaribles.sku+'&enable_avatars='+userVaribles.options.reviews.enable_avatars+'&include_subrating_breakdown=1'+apiVersion;
      }else{
        for(var x = 0; x<carouselType.length; x++){
        ...
        if(storeApi == 1){
          apiUrl= userVaribles.api+'/timeline/data?type=store_review&store='+userVaribles.store+'&sort=date_desc&page=1&per_page='+extraNum+'&sku='+userVaribles.sku+'&enable_avatars='+userVaribles.options.reviews.enable_avatars+'&include_subrating_breakdown=1'+apiVersion+finalString;
        }else{
          apiUrl= userVaribles.api+'/timeline/data?type=product_review&store='+userVaribles.store+'&sort=date_desc&page=1&per_page='+extraNum+'&sku='+userVaribles.sku+'&enable_avatars='+userVaribles.options.reviews.enable_avatars+'&include_subrating_breakdown=1'+apiVersion+finalString;
        }
...
function reviewsio_processAjax(userVaribles, ajaxData, dataType){
  ...
  for(var i = 0; i< objReview.length; i++){
      ...
      var verified = "";
      if(objReview[i]._source.order_id || objReview[i]._source.reviewer_desc === "Verified Buyer" || objReview[i]._source.verified_by === 'author'){
      ...
      // if( userVaribles.options.reviews.tags){
      //   hasTags = false;

      //   for(tag in setTagsArray){
      //     for(revTag in review.tags){
      //       // console.log(review.tags[revTag].tag)
      //       if(review.tags[revTag].tag.includes(setTagsArray[tag])){
      //         hasTags = true;
      //         break;
      //       }
      //     }
      ...
      if(userVaribles.options.reviews.hide_empty_reviews){
        ...
        var hideEmptyReview = false
        // if(review.comments == "" && !review.images){
        //   hideEmptyReview = true;
        // }
      }
